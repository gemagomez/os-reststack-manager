...
logger = logging.getLogger('quantum_tenant_net')

def quantum_tenant_net(net_name, subnet_name, cidr, opts_tenant_name, opts_shared, opts_dhcp, opts_dnsservers, opts_router, dhcp_ip_start, dhcp_ip_end, os_username, os_password, os_tenant_name, os_auth_url, os_region):
    keystone = ks_client.Client(username=os_username,
                                password=os_password,
                                tenant_name=os_tenant_name,
                                auth_url=os_auth_url,
                                region_name=os_region)
    quantum = client.Client(username=os_username,
                            password=os_password,
                            tenant_name=os_tenant_name,
                            auth_url=os_auth_url,
                            region_name=os_region)
    ...
    for tenant in [t._info for t in keystone.tenants.list()]:
        if (tenant['name'] ==
            (opts_tenant_name or os_tenant_name)):
            tenant_id = tenant['id']
    ...
    if len(networks['networks']) == 0:
        logger.info('Creating network: %s',
                     net_name)
        network_msg = {
            'network': {
                'name': net_name,
    ...
    if len(subnets['subnets']) == 0:
        logger.info('Creating subnet for %s',
                     net_name)
        subnet_msg = {
            'subnet': {
                'name': subnet_name,
    ...
    if opts_router:
        routers = quantum.list_routers(name=opts_router)
        if len(routers['routers']) == 0:
            ...
            if len(ports['ports']) == 0:
                logger.info('Adding interface from %s to %s',
                             opts_router, subnet_name)
